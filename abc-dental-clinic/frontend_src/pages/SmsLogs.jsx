...
export default function SmsLogs() {
  ...
  const load = async () => { const {data} = await api.get("/sms/logs?limit=200"); setLogs(data); };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-extrabold" style={{fontFamily:"Manrope"}}>SMS Reminders</h1><p className="text-slate-500 text-sm mt-1">Delivery logs & manual trigger. Auto-scheduler runs daily at 20:00 IST.</p></div>
      ...
      {logs && logs.length > 0 && logs.some(l => l.status === "simulated") && (
        <Card className="card-premium p-4 border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900">
          <div className="text-sm text-amber-800 dark:text-amber-200">
            <b>Twilio credentials not configured.</b> SMS is running in simulated mode (logged only). Add <code>TWILIO_ACCOUNT_SID</code>, <code>TWILIO_AUTH_TOKEN</code>, <code>TWILIO_FROM_NUMBER</code> to <code>backend/.env</code> and restart backend to enable real SMS.
          </div>
        </Card>
      )}
      ...
      <Card className="card-premium overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {logs && logs.length === 0 && <tr><td colSpan={4} className="py-12 text-center text-slate-500"><MessageSquare className="w-10 h-10 mx-auto mb-2 opacity-40"/>No SMS sent yet</td></tr>}
              {logs && logs.map(l => (
                <tr key={l.id} className="border-b border-slate-100 dark:border-slate-800" data-testid={`sms-log-${l.id}`}>
                  ...
                  <td className="px-6 py-3 max-w-md truncate">{l.body}</td>
