    ...
    try {
      if (editing) {
        ...
        if (changed) {
          ...
          toast.success("Rescheduled. Send SMS to inform patient →");
          setOpen(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          }} className="hidden md:inline-flex">📱 Send Tomorrow's SMS</Button>
                      ...
                      <div key={a.id} className={`p-4 rounded-xl border ${STATUS_COLORS[a.status] || "bg-slate-50 border-slate-200"} dark:bg-slate-800 dark:border-slate-700`} data-testid={`appt-card-${a.id}`}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            {a.reminder_sent_at && (
                              <span title={`SMS sent ${new Date(a.reminder_sent_at).toLocaleString()}`}
                        ...
                        <div className="flex gap-2 mt-3 text-xs flex-wrap">
                          ...
                          <button onClick={async()=>{
                            try { await api.post(`/appointments/${a.id}/send-reminder`); toast.success("SMS reminder sent"); load(); }
                            catch (e) { toast.error(formatApiError(e.response?.data?.detail)); }
                          }} className="text-[#0D9488] hover:underline font-semibold" data-testid={`appt-sms-${a.id}`}>{a.reminder_sent_at ? "Resend SMS" : "SMS"}</button>
        ...
        <DialogContent className="rounded-3xl">
          ...
          <div className="space-y-4 mt-2">
            <div className="text-sm">
              An SMS will be sent to <b>{reasonModal?.patient_name || "the patient"}</b> about the new slot{" "}
          ...
          <DialogFooter>
            ...
            <Button onClick={sendRescheduleSms} disabled={!reasonPick} data-testid="send-reschedule-sms" className="bg-[#0D9488] hover:bg-[#0F766E]">Send SMS</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
