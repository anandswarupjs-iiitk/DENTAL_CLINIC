...
def generate_invoice_pdf(invoice: dict, patient: dict, clinic: dict) -> bytes:
    ...
    # Header
    header_data = [[
        Paragraph(f"<b>{clinic.get('name', 'ABC Dental Clinic')}</b>", h1),
    ...
    # Bill To
    story.append(Paragraph("<b>BILL TO</b>", ParagraphStyle("bt", parent=body, textColor=TEAL)))
    story.append(Paragraph(
        f"<b>{patient.get('full_name','')}</b><br/>"
        f"Patient ID: {patient.get('patient_id','')}<br/>"
        f"{patient.get('address','')}, {patient.get('city','')}<br/>"
        f"Phone: {patient.get('phone','')} · Email: {patient.get('email','') or '—'}",
        body,
    ))
    story.append(Spacer(1, 12))
    ...
    tot = [
        ...
        ["Tax", f"₹{float(invoice.get('tax', 0)):.2f}"],
        ["Grand Total", f"₹{float(invoice.get('grand_total', 0)):.2f}"],
    ]
    ...
    # Payment Info
    story.append(Paragraph(
        f"<b>Payment Method:</b> {invoice.get('payment_method','—')}<br/>"
        f"<b>Paid:</b> ₹{float(invoice.get('paid_amount',0)):.2f} · "
        f"<b>Balance:</b> ₹{float(invoice.get('balance',0)):.2f}",
        body,
    ))
    story.append(Spacer(1, 20))
    story.append(Paragraph(
        f"Thank you for choosing {clinic.get('name','ABC Dental Clinic')}. "
